...
func TestParseCatalog_RoundTrip(t *testing.T) {
	...
	entries, err := indexdata.ParseCatalog(want)
	if err != nil {
...
func TestParseCatalog_NormalizesBOMAndCRLF(t *testing.T) {
	...
	a, err := indexdata.ParseCatalog([]byte(src))
	if err != nil {
		t.Fatal(err)
	}
	b, err := indexdata.ParseCatalog([]byte(crlf))
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseCatalog_Empty(t *testing.T) {
	entries, err := indexdata.ParseCatalog(nil)
...
func TestParseCatalog_Broken(t *testing.T) {
	...
	for _, c := range cases {
		_, err := indexdata.ParseCatalog([]byte(c.src))

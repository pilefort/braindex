...
func TestNewsBrowser(t *testing.T) {
	...
	if out, err := exec.Command("python", "-c", "import playwright.sync_api").CombinedOutput(); err != nil {
		t.Skipf("Python playwright が無いため Skip: %v %s", err, out)
	}
	dir := t.TempDir()
